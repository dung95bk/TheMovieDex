...
class LocationPageProvider extends ChangeNotifier {
  List<GuideModelDomain> listLocation = List<GuideModelDomain>();
  ...
  void firstCallData(GuideModelDomain locationData) {
    this.mapData = locationData;
    if (listLocation.length == 0 && !loadingMore) {
      getListMap();
    }
  }
  ...
  void getListMap() async {
    ...
    if (response != null && response.statusCode == 200) {
        ...
        listData.forEach((element) {
          listLocation.add(GuideModelDomain.fromData(
              id: element.id,
        ...
        print("data: ${listLocation.length}");
        notifyListeners();
    } else {
      allowLoadMore = false;
    }
